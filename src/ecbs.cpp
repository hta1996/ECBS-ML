...
inline bool cmp(const ECBSNode* a,const ECBSNode* b)
{
	return a->depth<b->depth;
}

void ECBS::printFeature2()
{
	vector<vector<ECBSNode*>> bin;
	for(int i=0;i<=4;i++)
	{
		vector<ECBSNode*> _;
		_.clear();
	...
	int max_d_to_optimal=0;
	int max_depth=0;
	int T_limit=10000;
	int T_node=0;
	//labeling nodes
	allNodes_table.sort(cmp);
	int prev_depth=0;
    for(auto node:allNodes_table)
 	{
		//if (node->depth<prev_depth)cerr<<"Error ";
		//cerr<<node->depth<<" ";
        ...
        if(!node->in_focal_list)continue;
		int label=4;
		if(node->d_to_optimal<10)label=0;
		else if(node->d_to_optimal<30)label=1;
		else if(node->d_to_optimal<60)label=2;
		else if(node->d_to_optimal<1000000)label=3;
		if(node->d_to_optimal<1000000)max_d_to_optimal=max(node->d_to_optimal,max_d_to_optimal);
		node->d_to_optimal=label;
		max_depth=max(node->depth,max_depth);
		bin[label].push_back(node);
		T_node++;
		if(T_node>=T_limit)break;
	}
	max_d_to_optimal--;
	cerr<<featFile<<endl;
    freopen(featFile.c_str(),"w",stdout);
	int sign=1,cnt=0;
	vector<double> depth_weight(2+max_depth,0);
	vector<double> node_weight;
	node_weight.clear();
	for(int i=1;i<=max_depth;i++)depth_weight[i]=exp(-(i-1)/(0.6*max_depth)*1.61);
	...
	for(int i=0;i<=4;i++)
		for(int j=i+1;j<=4;j++)
		{
			...
			for(auto node1:bin[i])
				for(auto node2:bin[j])
				{

					bool f1=node1->dfs_l<=node2->dfs_l&&node2->dfs_r<=node1->dfs_r;
					bool f2=node2->dfs_l<=node1->dfs_l&&node1->dfs_r<=node2->dfs_l;
					if(f1||f2)continue;
					printf("%d ",-1*sign);
					int idx=0;
        			for(auto x:node1->feature)
					{
						printf("%d:%.6lf ",idx+1,(x-node2->feature[idx])*sign);
						idx++;
					}
					puts("");
					cnt++;
					//cerr<<node1->depth<<" "<<node2->depth<<" "<<depth_weight[node1->depth]*depth_weight[node2->depth]<<" ";
					sum_of_weight+=depth_weight[node1->depth]*depth_weight[node2->depth];
					node_weight.push_back(depth_weight[node1->depth]*depth_weight[node2->depth]);
					sign*=-1;
				}
		}
	freopen((featFile+"Weight").c_str(),"w",stdout);
	for(int i=0;i<cnt;i++)
	{
		if(!solution_found)break;
		printf("%.4lf\n",10000./sum_of_weight*node_weight[i]);
	}
    fclose(stdout);
...
void ECBS::collectFeature(ECBSNode* node)
{
	...
	node->num_of_collisions=count_CA;

    features.push_back(count_CA);
    //features.push_back(count_CA*1.0/num_of_agents);
    
    //Current sum of cost, 7
    //features.push_back(node->g_val*1.0/sum_of_cost_initial); //old
	features.push_back(node->g_val*1.0/num_of_agents);
    ...
    //and difference 10
    //features.push_back((node->g_val-min_sum_f_vals)*1.0/sum_of_cost_initial);//old
	features.push_back((node->g_val-min_sum_f_vals)*1.0/num_of_agents);
	...
	features.push_back((node->g_val-sum_of_optiaml_path_cost)*1.0/num_of_agents);
...
void dfs_searchtree(ECBSNode* x,int &dfs_index)
{
	//cerr<<dfs_index<<" ";
	//cerr<<dfs_index<<"("<<x->depth<<") ";
	if(x==nullptr)return;
	x->dfs_l=dfs_index++;
	dfs_searchtree(x->child1,dfs_index);
	dfs_searchtree(x->child2,dfs_index);
	x->dfs_r=dfs_index++;
}
...
bool ECBS::run(list<agent_info> ma_info, list<Path*> in_other_paths)
{
	...
	ECBSNode* root_node=open_list.top();
	clock_t time_mark;
	int count_node_1=0;
	while (!open_list.empty())
	{
		count_node_1++;
		//if(count_node_1%20==0)cerr<<count_node_1<<" ";
		...
		{  
			...
			if(!solution_found)time_mark=loop_runtime;
			solution_found = true;
            ...
            printResults();
            count_solution_node++;
            if(count_solution_node>=num_of_solution_to_collect||testing==1)
                break;
            continue;
		...
		if(generateChild(n1, ma_info, in_other_paths))curr->child1=n1;else curr->child1=nullptr;
        n1->delta_collisions=n1->num_of_collisions-curr->num_of_collisions;
		paths = copy;
		if(generateChild(n2, ma_info, in_other_paths))curr->child2=n2;else curr->child2=nullptr;
...
//    cerr<<"to start dfs!"<<endl;
	if(testing!=1)
	{
		int dfs_index=0;
		dfs_searchtree(root_node,dfs_index);
